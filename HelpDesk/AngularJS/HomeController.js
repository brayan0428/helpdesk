        ...
        $scope.GuardarTicket = function () {
            ...
            $http({
                ...
                if (data[0].Result == 'True') {
                    ...
                    $timeout(function () { window.location.reload(true); }, 5000);
                } else {
       ...
       //Funcion para Consultar Tickets
        ...
        $scope.ConsultaTipoSoportes2 = function (TipoSoporte, TipoAccion, Responsable) {
            ...
            $http({
                ...
                IdResponsable   = data[0].Responsable;
                $("#SelTipoTicket").empty();
        ...
        $scope.ActualizarEtapa = function () {
            ...
            $http({
                method: 'GET',
                url: 'Consulta_TipoSoportes_Id?idTipoSoporte=' + Op,
                data: $.param({}),
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            }).success(function (data) {
                if (TipoAccion == 1) {
                    ...
                    if (TipoSolicitud == 2) {
                        ...
                        NivelResponsable = "URGENTE";
                        if (Responsable == 'JCARDALES' || Responsable == 'ARODRIGUEZ' || Responsable == 'STRESPALACIOS') {
                            estado = '13'
                        }
                    }
                    //
...
HelpDesk.controller('TicketsCitados', function ($scope, $http, $timeout, $interval) {
    try {
        ...
        $scope.AsignarValTicket = function (IdReunion,CodTicket,Necesidad,Justificacion) {
            $("#mc_CodTicket").val(CodTicket);
            $("#mc_IdReunion").val(IdReunion);
        ...
        $scope.ValidarAsistencia = function () {
            ...
            var IdReunion = $("#mc_IdReunion").val();
            ...
            if (Resp == '1') {
                ...
                $http({
                    ...
                    if (data[0].Result == 'True') {
                        ...
                        NotificacionPopup('error', 'Error de Validación', data[0].Motivo);
                    }
                })
           .finally(function () {
    ...
    try {
        ...
        $scope.ConsultarTicketsAsignados = function () {
            $http({
                ...
                $scope.CantMedia = 0;
                $scope.CantBaja = 0;
                $scope.CantSolucionados = 0;
                $scope.CantPendientes = 0;
                $scope.Tickets_Asignados = JSON.parse(data);
                angular.forEach($scope.Tickets_Asignados, function (value, key) {
                    if (value.Prioridad == 'URGENTE' && (value.Estado == '13' || value.Estado == '21')) {
                        $scope.CantUrgente += 1;
                    }
                    if (value.Prioridad == 'ALTA' && (value.Estado == '13' || value.Estado == '21')) {
                        $scope.CantAlta += 1;
                    }
                    if (value.Prioridad == 'MEDIA' && (value.Estado == '13' || value.Estado == '21')) {
                        $scope.CantMedia += 1;
                    }
                    if (value.Prioridad == 'BAJA' && (value.Estado == '13' || value.Estado == '21')) {
                        $scope.CantBaja += 1;
                    }
                    if (value.Estado == '22') {
                        $scope.CantPendientes += 1;
                    }
                    if (value.Estado == '6' || value.Estado == '17' || value.Estado == '18' || value.Estado == '19' || value.Estado == '20'  || value.Estado == '23') {
                        $scope.CantSolucionados += 1;
                    }
                });
            })
           .finally(function () {
...
HelpDesk.controller('Tickets_Calificacion', function ($scope, $http) {
    try {
        $scope.GuardarCalificacion = function () {
            var CodTicket       = $("#CodTicket").val();
            var TRespuesta      = $("#TRespuesta").val();
            var TAtencion       = $("#TAtencion").val();
            ...
            $http({
                ...
                if (data[0].Result == 'True') {
                    ...
                    NotificacionPopup('error', 'Error de calificacion', data[0].Msn);
                }
            });
        }
        $scope.CancelarCalificacion = function () {
            ...
            var Observacion = $("#Observacion").val();
            if (Observacion.trim() == '') {
                NotificacionPopup('error', 'Error de calificacion', 'Debe ingresar una observación');
                return;
            }
            $http({
                method: 'POST',
                url: 'CancelarCalificarTicket',
                data: $.param({
                    'CodTicket': CodTicket,
                    'Observacion': Observacion
                }),
